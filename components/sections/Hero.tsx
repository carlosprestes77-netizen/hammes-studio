...
export default function Hero() {
  ...
  // 3D monolith rotation — statue spins on Y axis as user scrolls
  const rotateYRaw = useTransform(scrollYProgress, [0, 1], [0, 32]);
  const rotateY = useSpring(rotateYRaw, { damping: 28, stiffness: 70 });

  // Slight tilt for depth
  const rotateXRaw = useTransform(scrollYProgress, [0, 1], [0, -6]);
  const rotateX = useSpring(rotateXRaw, { damping: 28, stiffness: 70 });

  // Scale up slightly so statue becomes more prominent mid-scroll
  const scaleRaw = useTransform(scrollYProgress, [0, 1], [1.08, 1.22]);
  const scale = useSpring(scaleRaw, { damping: 28, stiffness: 70 });

  const textY = useTransform(scrollYProgress, [0, 1], ["0%", "12%"]);
  const opacity = useTransform(scrollYProgress, [0, 0.55], [1, 0]);

  return (
    // Section is 180vh so the sticky statue is visible and spinning for longer
    <section ref={ref} id="hero" className="relative bg-paper-900" style={{ minHeight: "180vh" }}>
      {/* Sticky viewport: pins to top while section scrolls */}
      <div className="sticky top-0 h-screen overflow-hidden flex items-end pb-20 lg:pb-28">

        {/* 3D perspective container for the rotating monolith */}
        <div className="absolute inset-0 z-0" style={{ perspective: "1400px", perspectiveOrigin: "50% 40%" }}>
          <motion.img
            src={HERO_BG}
            alt=""
            aria-hidden
        ...
        {/* Gradient overlays */}
        <div className="absolute inset-0 z-[1] bg-gradient-to-t from-paper-950/95 via-paper-950/55 to-paper-950/30 pointer-events-none" />
        <div className="absolute inset-0 z-[1] bg-gradient-to-r from-paper-950/70 via-transparent to-transparent pointer-events-none" />

        {/* Ghost VINCIT */}
        <div
          className="absolute inset-0 z-[2] flex items-center justify-end pr-8 lg:pr-20 pointer-events-none select-none overflow-hidden"
          aria-hidden
        >
          <p
